...
/*   Updated: 2018/11/23 11:35:26 by pduhard-    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
static char	*check_nine(char **nb, int i, int prec)
{
	int		nine_count;
	...
	while (i > -1)
	{
		if (nb[0][i] != '9' && nine_count < (prec < 15 ? 15 : prec))
			return (NULL);
		else
			nine_count++;
		i--;
...
static char	*get_floats(long double n, int prec)
{
	...
	while (n > 0.0)
	{
		...
		if (i > 1 && nb[i - 1] != '9' && nb[i - 2] == '9')
			if (check_nine(&nb, i - 2, prec) != NULL)
				n = 0.0;
	}
	nb[i] = '\0';
